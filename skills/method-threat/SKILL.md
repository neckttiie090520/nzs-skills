...
Running it after is a review, and `method-security` is the skill for that.
...
The same ranking `method-security` uses, for the same reason: a model that ranks
...
2. **A task**, with the review role that will check it (`method-review` will
   select `security`) or the code that must be written.
3. **A guard**, when the threat is a class that recurs rather than a one-off —
   handed to `method-guard`, red-on-mutation before it counts.
