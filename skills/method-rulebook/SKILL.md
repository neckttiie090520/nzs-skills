...
- [ ] **The claim verified in the wrong medium.** Did you confirm a rendered fact
  ...
  claim where it is true (§ delegate to `method-witness` for the medium map).
...
- `method-witness` is this gate made medium-specific: it owns HOW to settle each
  kind of claim. Call it before any "done."
- `method-tripwire` enforces law 2: it will not wire in a guard not seen failing.
- `method-launch` enforces laws 3 and 5: honest bounds, ratchet-only budgets, and a
  commit body that may not assert a fix `method-witness` has not settled.
- `method-gauntlet` enforces law 4: every finding is fix-now, record-with-price, or
