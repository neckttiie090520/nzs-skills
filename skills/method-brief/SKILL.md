...
itself. You are not deciding whether to build it (`method-greenlight`) or how
(`method-blueprint`) — you are making the request legible enough that those gates can
run.
...
- **Audience** — the specific person, named by what they are trying to do, not
  ...
  *"admins."* If you cannot name who, you are describing a feature you wanted,
  not one someone needs — send it to `method-fieldwork`.
...
checkable statement. The test is method-greenlight's test: *"make it good"* is not a
...
- **Flag, do not guess.** If a factor cannot be scored without evidence you do
  ...
  hand that factor to `method-fieldwork` or `method-scout`. A guessed
...
- **For whoever builds** — the mechanism and the constraint: what changes, where,
  what must stay true. This is the bridge to `method-blueprint`.
...
- **`method-greenlight`** — *should we build it at all?* Once an item has an
...
- **`method-blueprint`** — *how?* After decide says build, the shaped item and its
  observables become the spec's §1 and §4.
- **`method-fieldwork` / `method-scout`** — any factor or audience you had to
...
- **A roadmap item nobody asked for.** An item with no named audience in §1 is a
  preference of the writer's. Send it to `method-fieldwork` for evidence that
