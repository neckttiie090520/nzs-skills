...
1. **Read `.method/config.yml`.** You need `verify.check` and `verify.guards`
   ...
   run `method-onboard` first. Do not invent a verify command; a spec whose
...
3. **Check the extract gate.** If the request carries a reference — a URL, a
   ...
   plan yet. `method-research` fetches it, `method-extract` turns it into rules.
...
- **§4 The decisions.** One heading per decision, each stating the rule in
  ...
  *"any other admin page ≤ 150 words"*, not *"keep pages short"*. This is the
  hand-off to `method-guard` — a decision phrased as a number is a decision that
...
- **To cite a raw reference.** Only rules that came through `method-extract`,
