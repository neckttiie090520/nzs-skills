...
`method-tripwire` fires when a bug has recurred TWICE. `method-gauntlet` needs a
...
twice, hand to `method-tripwire` — the second occurrence is when a rule is born.
Settle the fix in its medium with `method-witness`; a bug is not fixed because
the code changed.
