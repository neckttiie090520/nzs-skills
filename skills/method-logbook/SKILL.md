...
description: >-
  ...
  entry, never as silence. Records verdicts from method-greenlight, priced declines
  from method-blueprint and method-gauntlet, and postmortem outcomes. Use when a
...
- `method-greenlight` → the verdict (build/buy/defer/don't) with its assumption-map
  reasoning; defer becomes Queued, don't becomes Refused-with-price.
- `method-blueprint` §7 and `method-gauntlet`'s record-with-price bin → Refused/Queued
  entries with their prices.
- `method-tab` → the volume trigger a Queued item revisits at.
- `method-autopsy` → moves a Shipped entry to Refused (killed) or annotates it
