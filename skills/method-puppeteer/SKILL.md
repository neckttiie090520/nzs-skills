...
Model output is caller data, and `method-trapdoor`'s sink rules apply unchanged.
...
- **Never put a secret in a prompt.** Not the key, not the connection string.
  `method-vault` applies unchanged; a prompt is a log with extra steps.
