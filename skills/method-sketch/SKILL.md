...
- **Draw every state you intend to ship** — empty, loading, one item, many,
  ...
  drawing short (hand the states themselves to `method-stress-test`).
...
- A measure or ratio you want to hold forever → `method-tripwire`: a script that
  reads the computed value and fails the build beats a person re-measuring.
- Any "it renders / it is applied" claim → `method-witness`, which owns the medium
  map and the stale-chunk rule.
- The rendered result itself → `method-gauntlet`'s design-jury role, which measures
