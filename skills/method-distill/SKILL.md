...
description: >-
  ...
  method-scout / method-mimic into method-tripwire and method-blueprint. Use after studying any
...
The counter is structural, not a warning. **`method-blueprint` may cite extracted rules only, never
...
Read whichever you have: a findings file from `method-scout`, the measurement docs from
`method-mimic`, or the reference itself. Also read `.method/config.yml` (`references`, and the
existing parameters) and `.method/budgets.yml`.

If the observation is not written down anywhere yet, stop and run `method-scout` first. You
...
extract** — send it back through `method-scout`. Taste is allowed to drift, but only on
...
- **adopt-as-guard-candidate** → hand to **`method-tripwire`**, which will write the script, prove
...
Then: **`method-blueprint`** cites these rules by name. That citation is the only legal route from a
reference into the codebase.

## 5. Refuse

- **To output a component, a snippet, or any code as your product.** Producing the artefact is
  `method-mimic`'s job. Your product is a rule. If you find yourself writing JSX, you are in the
  wrong skill.
...
candidate handed to `method-tripwire`, or declined-with-price). Count adopted vs not-adopted
