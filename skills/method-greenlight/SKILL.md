...
description: >-
  ...
  as an observable. Use before method-blueprint for anything of real size, or when the
...
send it to `method-scout` for evidence first.
...
These are the seeds `method-autopsy` later instruments. A feature with no kill
criterion is a feature nobody will ever decide to remove, because there is no
threshold to point at — it just accretes.

## 4. The verdict

One of four, and it goes to `method-logbook` (never left in conversation):

- **build** — assumptions either evidenced or cheap-to-test-and-tested; success
  is an observable; kill criteria set. Hand to `method-tab` for the unit
  cost, then `method-blueprint`.
