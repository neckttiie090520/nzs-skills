...
description: >-
  ...
  whether it was built well. Runs after method-ship, at the postmortem trigger a
...
- Name the assumption and the test `method-decide` recorded for it.
...
- **Kill or simplify** — the criterion is not met. Recommend the simpler shape,
  and — as `method-decide` requires — **name what is LOST by simplifying**, not
...
- If the postmortem revealed a class of bug (a fallback hiding a broken feature; a
  green test proving only the rule that fired), hand it to `method-guard` — a
