...
First apply `method-rulebook`'s anti-theatre gate to your own output. Then, for
...
**A guard claim — seen failing, then passing.** Delegate to `method-tripwire`: a
...
reason**). Hand the verified set to `method-launch` or the caller; hand any
