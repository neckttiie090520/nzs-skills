...
the file being named `server`. Delegate to `method-security` for the reachability
argument.

## The guard, because this recurs

This is a recurring class, so it earns an executable guard rather than a rule
people remember. Delegate to `method-guard` and add it to `verify.guards`:
...
- **To store a secret in memory tooling.** `nzs-learn` records shapes and taste,
  never values.
