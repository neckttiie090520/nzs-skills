...
name: nzs-roundtable
description: >-
  One roundtable, two jobs — pick by scope, not by asking. RESOLVE a request
  ...
  says grill this / ตบหน่อย / roundtable this / resolve this yourself / what
  would a CTO say / play devil's advocate / คุยกันเอง.
---

# nzs-roundtable

One engine, two jobs. Both start the same way — cast the roundtable — and
...
## The roundtable — the mechanism both jobs share
...
Find every unresolved branch, then seat the roundtable against each one
...
`method-decide`'s assumption map already has a slot for exactly this — risk ×
...
Seat the roundtable once, against the whole decision, not against a branch.
This is the job for a decision that "feels settled too easily" — use
`method-review` instead if what you actually have is a diff to inspect.

**Output — a single verdict:**

- the decision, one line
- what changed because of the roundtable — if nothing changed, say so
...
- **A CTO seat that cannot say no.** If the answer was decided before the
  roundtable sat, do not seat one.
