...
`handoff`. Every stage below cites `method-rulebook`; no "done" in this loop may be
narrated instead of shown.

**If `.method/config.yml` does not exist, run `method-groundwork` first.** Every skill
...
- **A URL or repo in the request** → `method-scout` (and `method-mimic` too if
  the ask is to look or behave like it).
- **A local folder of images or documents** ("อยากได้ ui แนวนี้เลย …\Downloads\2")
  → `method-mimic` on those references.
- **"make it like X" / "world-class" / เลียนแบบ / แนว naming a product class**, with
  no reference in hand → `method-scout` first to fetch the actual references,
  because cloning a memory of Canva is cloning a hallucination.
- **After research or clone, `method-distill` runs before `method-blueprint`** — so the
...
integration), route to `method-greenlight` BEFORE `method-blueprint` — build / buy /
...
hands to `method-tab` (the unit cost, the cap, the volume it breaks at)
and then to `method-blueprint`; every verdict is filed by `method-logbook` so no
...
1. **Acquire** (only if routed above) — `method-scout` / `method-mimic` /
   `method-distill`. Produces cited findings and updated `config.references`.
   Tier: architect for judgment, mechanical for bulk transcription.

2. **Plan** — `method-blueprint` on the **architect** tier, for anything past one-file
...
4. **Review** — `method-gauntlet`, 2–3 rounds, **rotating the role each round** so
...
5. **Verify** — `method-witness` before any milestone is called done. Each claim
   settled in its medium, or downgraded to "attempted, unverified."

6. **Guard** — `method-tripwire` fires on recurrence: the *second* time a bug of one
   class is fixed, a guard is proposed, seen failing, and wired in.

7. **Ship** — `method-launch`: production-precondition check, migration end-state,
   honest bounds, ratchet check, commit body, deploy confirmed in its medium.

8. **Postmortem** — after ship, at the feature's recorded trigger, `method-autopsy`:
   ...
   and updates `method-logbook` with the outcome.
...
- To plan a reference-bearing request before acquisition has produced cited
  findings.
- To plan a feature of real size before `method-greenlight` has ruled on it and
  stated its success as an observable.
- To mark a milestone done before `method-witness` passes.
...
- **Before Decide** — `method-brief` shapes a raw request into who it is for and
  what they can do afterwards; `method-longlist` generates the options including
  do-nothing; `method-fieldwork` goes and finds out what is true when the
  answer is "nobody knows". All three hand into `method-greenlight`, which rules.
- **Inside Build** — `method-craft` before any implementation: find the seam, make
  the smallest change that solves the real problem, trace the blast radius.
- **Inside Review and Verify** — `method-stress-test` for anything holding money,
  state or more than one step; `method-sketch` for any surface a person looks
