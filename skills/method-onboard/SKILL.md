...
description: >-
  ...
  time any other method skill runs in a project that has never onboarded; invoke
  directly to re-onboard or to add a reference.
---
...
mediums:                             # where a claim of each kind is settled
  ...
  persistence:                       # TWO connections, and prod is the one
    local:     "<db read command>"   # that matters: method-ship verifies the
...
roles:        [<the roster that fits this domain>]
              # must include every role method-review's table can select,
