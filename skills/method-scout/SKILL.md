...
`for:` and `as_of:`) and `handoff`. If `.method/config.yml` does not exist, run `method-groundwork`
...
`method-distill` and is the gate that stops cargo-culting. Your job ends at sourced evidence.
...
5. Hand the findings into `method-gauntlet`'s triage like any other round: fix-now /
...
- Findings that might become project rules → **`method-distill`**. It is the only path from a
  reference into a spec: `method-blueprint` may cite extracted rules, never raw admiration.
- A visual or interaction reference the project intends to reproduce → **`method-mimic`**.
