...
recommendation; the recommendation is `method-decide`'s to make from what you
find.
...
- **Unsupported assumptions** — beliefs the plan would rest on that no finding
  ...
  caught. These are the seeds of `method-decide`'s assumption map.
...
- **`method-research`** — every unanswered question and unsupported assumption in
  §4 leaves as a named thing to go find out, with the cheapest source that would
  settle it where you can name one.
- **`method-decide`** — the themes, the JTBDs, and the §4 assumption list feed
