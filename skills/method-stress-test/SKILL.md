...
- [ ] Does the number in the message come from the same constant as the check?
      If it is typed twice, they will drift (hand to `method-craft`).
...
- A failure class you have now fixed twice → `method-tripwire`, so a build catches
  the third (the read-then-act race became exactly this kind of guard here).
- Every "it handles that" you are about to write → `method-witness`. Robustness
  claims are settled by running the broken case, not by reading the catch block.
- The findings themselves → `method-gauntlet`'s triage: fix-now, record-with-price,
