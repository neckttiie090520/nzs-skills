...
|---|---|---|
| a URL, repo, screenshot folder, "make it like X" | acquire | `method-scout` → `method-distill` |
| "should we", "worth it", "คุ้มไหม" | decide | `method-greenlight` |
| a feature described in prose | build | `method-blueprint` |
| a diff, PR, screen, "review this" | review | `method-gauntlet` |
| one broken thing, "why is X doing Y" | diagnose | `method-whodunit` |
| "ship", "deploy", "push" | ship | `method-launch` |
| "how do users", "is this real" | discover | `method-fieldwork` |
| credentials, money, other people's data, uploads, external input | build, but model it first | `method-lookout` → `method-blueprint` |
...
| "everything you've got", "sweep this", "hit it with everything" | sweep | `nzs-areyousure` |
...
**When the job is underspecified, `nzs-huddle`'s Resolve job comes before
...
**Its Judge job is not a review round.** `method-gauntlet` rotates disciplines
...
signal in the session**, and routing it to `nzs-scrapbook` costs one step and saves the
same mistake twice.
...
- **Where it can stop early.** Most jobs have a step whose result can end the
  work — a `method-greenlight` that says don't build, a `method-fieldwork` that
...
If the job is large enough to need its own standing loop, hand to `nzs-marathon`,
