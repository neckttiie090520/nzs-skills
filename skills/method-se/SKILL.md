...
4. **What is the real problem?** The reported symptom is often one instance.
   ...
   instance and note the class (that is `method-guard`'s recurrence trigger).
...
cannot reach, that is `method-guard`'s job: a guard that reads the source of
...
- A rule you keep re-explaining in review → `method-guard`, so a build fails
  instead of a person remembering. Duplicated constants across a boundary are
  the classic candidate.
- "The smaller change works" → `method-verify`. Simplest-first is a claim about
  behaviour, and it is settled in the medium, not by how confident the diff looks.
- The shape of the change once written → `method-review`, whose scrutinize role
