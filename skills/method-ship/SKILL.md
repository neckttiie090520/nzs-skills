...
- **Be verified in the medium of persistence**, per `method-verify`: read the
...
The body may not assert a fix that `method-verify` has not settled. If a claim is
...
- To write a commit body that asserts a fix `method-verify` has not settled.
